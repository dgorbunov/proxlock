...
float estimate_distance(uint8_t *local_steps, uint16_t local_steps_len, uint8_t *peer_steps,
		       uint16_t peer_steps_len, uint8_t n_ap, enum bt_conn_le_cs_role role)
{
	...
	return estimate_distance_using_phase_slope(mode_2_data, context.mode_2_data_index);
}
